    ...
    @push('before_scripts')
        <div class="col-md-12 well repeatable-element row m-1 p-2" data-repeatable-identifier="{{ $field['name'] }}">
            @if (isset($field['fields']) && is_array($field['fields']) && count($field['fields']))
                <div class="move move-element action-element"><span>#</span></div>

                <button type="button" class="close delete-element action-element"><span aria-hidden="true">×</span>
                </button>
                ...
                @foreach($field['fields'] as $subfield)
                    ...
                    @include($fieldViewPath, ['index' => 1, 'field' => $subfield])
                @endforeach
...
@if ($crud->fieldTypeNotLoaded($field))
    ...
    @push('crud_fields_styles')
        ...
        <style type="text/css">
            ...
            .container-repeatable-elements .move-element:hover {
                opacity: .9;
            }
        </style>
    @endpush
    ...
    @push('crud_fields_scripts')
        ...
        <script>
            $(function () {
                $(".container-repeatable-elements > div").sortable({
                    handle: ".move-element"
                });
            });
